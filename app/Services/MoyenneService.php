...
class MoyenneService
{
        ...
        foreach ($structureClasse as $matiereStructure) {
            ...
            foreach ($enrollments as $enrollment) {
                ...
                if (! $noteRecord) {
                    ...
                    $appreciation = $this->getSubectRemark($valeurNote);
                }
                ...
                $notesMatiere[$enrollment->id] = $valeurNote;
            }

            // 2. Calcul des Rangs, Min, Max et Moyenne de Classe pour CETTE matière
            $this->loadSubjectRankStats($classeId, $matiereStructure->matiere_id, $sequenceId, $notesMatiere);
        }

        return true;
    }

    private function loadSubjectRankStats($classeId, $matiereId, $sequenceId, $notes)
    {
        if (empty($notes)) {
            return;
        }
        ...
        foreach ($notes as $enrollmentId => $note) {
            ...
            Moyenne::where([
                ...
                'min_classe' => $min,
                'max_classe' => $max,
                'appreciation' => $this->getSubectRemark($note),
            ]);
    ...
    private function getSubectRemark($note)
    {
        ...
        foreach ($enrollments as $enrollment) {
            ...
            Bilan::updateOrCreate(
                ...
                [
                    ...
                    'moyenne' => $moyenneG,
                    'effectif_classe' => $enrollments->count(),
                    'mention' => $this->getMention($moyenneG),
                    'rang' => 0,
                ]
            );

            $moyennesGenerales[$enrollment->id] = $moyenneG;
        }

        $this->assignOverallRanks($classeId, $sequenceId, $moyennesGenerales);

        return true;
    }

    private function assignOverallRanks($classeId, $sequenceId, $moyennes)
    {
        if (empty($moyennes)) {
            return;
        }
        ...
        foreach ($moyennes as $enrollmentId => $moyenne) {
    ...
    public function calculerMoyennesTrimestrielles($classeId, $trimesterId)
    {
        ...
        foreach ($structureClasse as $matiere) {
            ...
            foreach ($enrollments as $enrollment) {
                ...
                $moyennesSeq = Moyenne::where('inscription_id', $enrollment->id)
                ...
                Moyenne::updateOrCreate(
                    ...
                    [
                        ...
                        'appreciation' => $this->getSubectRemark($valeurTrim),
                        'rang' => 0, // <--- AJOUTE CETTE LIGNE ICI
                    ]
                );

                $notesTrimestre[$enrollment->id] = $valeurTrim;
            }

            // Calcul des stats de classe pour le trimestre (Min, Max, Rang)
            $this->fillTermSubjectStats($classeId, $matiere->matiere_id, $trimesterId, $notesTrimestre);
        }

        return true;
    }

    private function fillTermSubjectStats($classeId, $matiereId, $trimesterId, $notes)
    {
        if (empty($notes)) {
            return;
        }
