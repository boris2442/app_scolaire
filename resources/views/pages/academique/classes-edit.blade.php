...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-xs text-muted-foreground tracking-tighter">Mise à jour des informations de la classe : {{ $classe->nom }}</p>
        </div>
    </div>
        ...
        <div class="bg-card p-6 rounded-xl border border-border shadow-sm">
            ...
            <form action="{{ route('settings.classes.update', $classe->id) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="cycle_id" class="w-full bg-secondary border-border rounded-lg text-sm px-3 py-2 mt-1" required>
                        <option value="">Sélectionner un cycle...</option>
                        @foreach ($cycles as $cycle)
                            <option value="{{ $cycle->id }}" {{ (old('cycle_id', $classe->cycle_id) == $cycle->id) ? 'selected' : '' }}>
                                {{ $cycle->nom }}
                            </option>
                        @endforeach
                    </select>
