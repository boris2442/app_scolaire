                ...
                <table class="w-full text-left">
                    ...
                    <tbody>
                        @foreach ($students as $student)
                            @php
                                $moyenne = $student->calculerMoyenneAnnuelle($actifYear);
                                $estAdmis = $moyenne >= $seuilReussite;
                            @endphp
                            <tr>
                                <td class="p-4">{{ $student->nom }}</td>
                                <td class="p-4 font-bold">{{ number_format($moyenne, 2) }}</td>
                                <td class="p-4">
                                    <span
                                        class="px-2 py-1 rounded text-xs {{ $estAdmis ? 'bg-success/20 text-success' : 'bg-danger/20 text-danger' }}">
                                        {{ $estAdmis ? 'ADMIS' : 'REDOUBLANT' }}
                                    </span>
                                </td>
                                <td class="p-4">
                                    <select name="decisions[{{ $student->id }}]" class="border rounded p-1">
