...
@section('content')
    @if ($errors->any())
        <div class="p-4 mb-4 bg-red-500/10 border border-red-500 rounded text-red-600 text-[10px] font-black ">
...
$currentClasseId = $eleve->inscriptions->where('annee_scolaire_id', $anneeActive->id)->first()
                    ...
                    <div>
                        <label class="text-[10px] font-bold  text-muted-foreground ml-1">Changer la classe</label>
                        <select name="classe_id"
                            class="w-full bg-secondary border-border rounded py-3 px-4 mt-1 text-sm font-black  outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                            required>
                ...
                <div class="bg-card p-8 rounded-2xl border border-border shadow-sm text-center">
                    ...
                    <p class="text-[9px] text-muted-foreground mt-2  font-bold">Laissez vide pour conserver la
                        photo actuelle</p>
                </div>
            </div>
        </div>
