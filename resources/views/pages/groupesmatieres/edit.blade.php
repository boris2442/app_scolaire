...
@section('content')
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-foreground mb-2">Modifier le Groupe de Matières</h1>
        <p class="text-sm text-gray-500 mb-8">Mettez à jour les informations de ce groupe pour organiser les matières sur le
            bulletin.</p>

        <form action="{{ route('admin.groupes.update', ['groupe' => $groupe->id]) }}" method="POST"
            class="bg-card p-6 rounded-2xl border border-border shadow-sm">
            @csrf
            @method('PUT')
        ...
        <input type="text" name="nom" required value="{{ old('nom', $groupe->nom) }}"
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('admin.groupes.index') }}"
                    class="px-6 py-3 text-sm font-bold text-gray-600 hover:text-foreground transition-colors">Annuler</a>
                <button type="submit"
                    class="px-8 py-3 bg-primary text-white rounded-xl font-bold text-sm shadow-lg shadow-primary/20 hover:scale-105 transition-all">
